...
def project_x_python(l,u):
    ...
    full_data=pd.read_csv('clinical_roi_meger_com.csv')

    clean_up_clinical([full_data])

    df=np.array(full_data)
    X_orig = df[:, :-1]
    Y_orig = df[:, -1]
    Y_til=Y_orig
...
parser.add_argument('--l', type=int, default = 1, help='what is the lower projected dim')
parser.add_argument('--u', type=int, default =30, help='what is the upper projected dim')
